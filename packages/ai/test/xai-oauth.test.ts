...
function refreshXaiForTest(refreshToken: string): Promise<OAuthCredentials> {
	return xaiOAuthProvider.refreshToken({ refresh: refreshToken, access: "old-access", expires: 0 });
}
...
describe("xAI OAuth device flow", () => {
	...
	it("uses the device grant, delays polling, and handles pending and slow_down", async () => {
		...
		const loginPromise = loginXaiForTest({ onAuth: (info) => authInfos.push(info) });
	...
	it("rejects a non-https verification_uri_complete even when verification_uri is https", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(async () =>
				jsonResponse(
	...
	it("prefers the prefilled verification_uri_complete link", async () => {
		vi.useFakeTimers();
		const startTime = new Date("2026-07-09T20:00:00Z");
		vi.setSystemTime(startTime);
		vi.stubGlobal(
	...
	it("times out when the device code expires", async () => {
		vi.useFakeTimers();
		const startTime = new Date("2026-07-09T20:00:00Z");
		vi.setSystemTime(startTime);
		const pollTimes: number[] = [];
		vi.stubGlobal(
			...
			vi.fn(async (input: unknown) => {
				if (requestUrl(input) === "https://auth.x.ai/oauth2/device/code") {
					return jsonResponse(deviceCodeResponse({ interval: 1, expires_in: 2 }));
				}
				pollTimes.push(Date.now());
	...
	it("refreshes tokens and preserves an unrotated refresh token", async () => {
		...
		expect(xaiOAuthProvider.name).toBe("xAI (SuperGrok / X Premium)");
		expect(xaiOAuthProvider.isSubscription).toBe(true);
