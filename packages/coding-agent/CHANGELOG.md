...
- Added "xAI (SuperGrok / X Premium)" subscription login to `/login` alongside the existing xAI API key login.
- Changed the default xAI model to grok-4.6.
- Added a footer label showing when the current model runs on a known subscription credential.
